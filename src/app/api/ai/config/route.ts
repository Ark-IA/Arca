...
import { AiError, PROVEEDORES, type AiProvider } from '@/lib/ai/types'
...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(
        'provider, model, system_prompt, is_active, auto_reply_enabled, auto_reply_max_per_conversation, auto_reply_channels, handoff_agent_id, handoff_message, unsupported_media_message, api_key, embeddings_api_key, transcription_api_key, transcription_model, transcription_base_url',
      )
...
export async function POST(request: Request) {
  try {
    ...
    if (!PROVEEDORES.includes(provider)) {
      return bad(`provider must be one of: ${PROVEEDORES.join(', ')}`)
    }
    ...
    // Igual con los canales: `null` significa que el formulario no los mandó,
    // y sobrescribir con una lista vacia dejaria al agente mudo en todas las
    // bandejas por guardar un cambio de otra cosa.
    if (autoReplyChannels !== null) {
      shared.auto_reply_channels = autoReplyChannels
    }
    if (rawEmbeddingsKey) {
    ...
    if (typeof body.unsupported_media_message === 'string') {
      shared.unsupported_media_message = body.unsupported_media_message.trim()
    }
