...
import type { ObjectDefinition, ViewDefinition, ViewType } from '@/types/objects';
...
export default function VistasPage() {
  ...
  const traer = useCallback(async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : 'No se pudieron cargar las vistas');
    }
  ...
  const crearVista = async () => {
    ...
    try {
      const objetos = createCustomObjectsManager(supabase, accountId);
      const { error } = await objetos.createView(objectId, { name: nombre.trim(), type: tipo });
      if (error) {
        toast.error(error);
        return;
      }
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="font-bold text-3xl tracking-tight">Vistas</h1>
                ...
                <Card key={v.id}>
                  <CardHeader className="pb-2">
                    <CardTitle className="flex items-center gap-2 text-base">
                      {v.isDefault && <Star className="size-3.5 text-primary" />}
                      {v.name}
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
      ...
      <Dialog open={abierto} onOpenChange={setAbierto}>
        <DialogContent className="sm:max-w-[420px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Una forma guardada de listar {objeto?.labelPlural ?? 'los registros'}.
            </DialogDescription>
          </DialogHeader>
