...
export default function PermisosPage() {
  ...
  const esAdmin = profile?.account_role === 'owner' || profile?.account_role === 'admin';
  ...
  const traer = useCallback(async () => {
    ...
    if (filas.length === 0) {
      const { error } = await permisosMgr.initializeDefaultPermissions(objectId);
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="font-bold text-3xl tracking-tight">Permisos</h1>
                ...
                {ACCIONES.map(({ puede, alcance, titulo: accion }) => (
                  <div key={puede} className="flex items-center gap-3">
                    <Checkbox
                      ...
                      disabled={!esAdmin}
                      onCheckedChange={(v) =>
                        editar(valor, { [puede]: Boolean(v) } as Partial<ObjectPermission>)
                      }
                    />
                    <Label
                      htmlFor={`${valor}-${puede}`}
