...
export function WhatsAppConfig() {
  ...
  const fetchConfig = useCallback(
    async (acctId: string, lineaPedida?: string | null) => {
      setLoading(true);
      try {
        ...
        const data =
          lineaPedida === NUEVA
            ? null
            : (lineas.find(
                (l) => l.id === (lineaPedida ?? lineaActivaRef.current)
              ) ??
              lineas[0] ??
              null);
        lineaActivaRef.current =
          lineaPedida === NUEVA ? NUEVA : (data?.id ?? null);
  ...
  async function handleSave() {
    ...
    if (!config && (!accessToken.trim() || !tokenEdited)) {
      toast.error(
        'El Access Token es obligatorio para la configuración inicial'
      );
      return;
    }
    ...
    try {
      ...
      if (tokenEdited && accessToken !== MASKED_TOKEN && accessToken.trim()) {
        ...
        toast.error(
          'Vuelve a ingresar el Access Token para guardar los cambios'
        );
        setSaving(false);
        return;
      }
      ...
      if (data.registered === false && data.registration_error) {
        toast.error(
          `Se guardó, pero Meta no pudo registrar el número: ${data.registration_error}`,
          { duration: 12000 }
        );
        ...
        toast.success(
          'Credenciales guardadas y verificadas. Se omitió el registro de mensajes entrantes (sin PIN): revisa el estado del registro más abajo.',
          { duration: 10000 }
        );
        setPin('');
      } else {
  ...
  async function handleTestConnection() {
    try {
      ...
      setConnectionStatus('disconnected');
      toast.error(
        'Falló la prueba de conexión. Revisa tu red e inténtalo de nuevo.'
      );
    } finally {
      setTesting(false);
    }
  ...
  async function handleVerifyRegistration() {
    ...
    try {
      ...
      if (data.live) {
        ...
        toast.error(
          'El número no está completamente registrado. Revisa las comprobaciones de abajo para ver qué paso falló.',
          { duration: 8000 }
        );
      }
      ...
      toast.error('No se pudo contactar el servicio de verificación.');
    } finally {
  ...
  async function handleReset() {
    ...
    try {
      ...
      toast.success(
        'Configuración eliminada. Ya puedes ingresar tus credenciales de nuevo.'
      );
      setConfig(null);
      setPhoneNumberId('');
      setWabaId('');
            ...
            <button
              ...
              className={
                l.id === lineaActiva
                  ...
                  : 'border-border text-muted-foreground hover:text-foreground rounded-full border px-3 py-1 text-xs transition-colors'
              }
            >
              {l.phone_number_id}
            </button>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
        ...
        <div className="space-y-6">
          ...
          {canEditSettings && lineas.length > 0 && lineaActiva !== NUEVA && (
            <div className="border-border flex items-center justify-between gap-3 rounded-lg border border-dashed px-4 py-3">
              <p className="text-muted-foreground text-xs">
                ¿Tienes otro número de WhatsApp para esta cuenta? Puedes
                conectarlo aparte, con sus propias instrucciones para el agente.
              </p>
              <Button
                variant="outline"
                size="sm"
